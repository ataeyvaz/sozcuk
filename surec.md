...
- **Ölçüm (eski paket):** 1,1 GB gömülü dil paketi, 371 MB torch, ~120 MB spacy/thinc/blis, 66 MB PyAV. Ayrıca eski sürüm gömülü paketleri ilk açılışta `~\.local\sharergos-translate` içine açıp kopyalıyordu → kullanıcıda **~3 GB**
...
- Eski sürümün `~\.local\sharergos-translate` klasörü (1,2 GB, artık kullanılmıyor) kullanıcı onayıyla silindi
